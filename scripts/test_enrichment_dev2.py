...
class SummaryEnrichment(CandidateEnrichment):
    ...
    def initital_rank_table(self):
        real_assoc = self.get_association(self.features_per_data)
        rt = init_rank_table(real_assoc)
        return rt


    def get_association(self, values_per_feature):
        summary_per_feature = self.get_summary_per_feature(values_per_feature)
    ...
    def get_summary_per_feature(self,values_per_feature):
        #if self.flat_index:
    ...
    def permuter(self, rank_table, n_permut):
        ...
        for rnd in np.random.rand(n_permut):
            ...
            s.name = self.value_name
            shift_fpd = self.features_per_data.drop(self.value_name,axis=1).join(s)
            assoc = self.get_association(shift_fpd)
